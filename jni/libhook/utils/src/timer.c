...
static struct event *event_queue = 0;
...
(
	...
	const struct itimerspec * value,   /* time to be set */
	struct itimerspec *       ovalue   /* previous time set (0=no result) */
)
{
...
void timer_cancel_all()
{
	...
	setitimer(ITIMER_REAL, &timeroff, 0);
...
void timer_cancel(timer_t timerid)
{
	...
	while (*ppevent) {
		if (*ppevent == event) {
			 ...
			 */
			if (event == event_queue && event->next != 0) {
				timerclear(&itimer.it_value);
				getitimer(ITIMER_REAL, &itimer);
...
{
	module_id = 0;
	return timer_create(CLOCK_REALTIME, 0, (timer_t *)timer_id);
}
...
{
	return timer_settime((timer_t)timer_id, 0, timer_spec, 0);
}
